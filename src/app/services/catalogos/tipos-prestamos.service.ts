...
export class TiposPrestamosService {
  ...
  async postTipoPrestamo(data: any): Promise<any> {
    ...
    if (tipo_prestamo.resultado) {
      await this.rootService.bitacora('tipo_prestamo', 'agregar', `creó el tipo_prestamo "${tipo_prestamo.data.nombre}"`, tipo_prestamo.data);
    }
  ...
  async putTipoPrestamo(id: number, data: any): Promise<any> {
    ...
    if (tipo_prestamo.resultado) {
      await this.rootService.bitacora('tipo_prestamo', 'editar', `editó el tipo_prestamo "${tipo_prestamo.data.nombre}"`, tipo_prestamo.data);
    }
    return tipo_prestamo;
  }
  ...
  async deleteTipoPrestamo(id: number): Promise<any> {
    ...
    if (tipo_prestamo.resultado) {
      await this.rootService.bitacora('tipo_prestamo', 'eliminar', `eliminó el tipo_prestamo "${tipo_prestamo.data.nombre}"`, tipo_prestamo.data);
    }
    return tipo_prestamo;
  }
}
